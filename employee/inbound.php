            ...
            <?php
                ...
                if (mysqli_num_rows($result)>0){
                    while($row1=mysqli_fetch_array($result)){
                        ...
                        $task=$row1['task'];
                        $query="select * from ship where name='$sname' and (stat='managing' or stat='pending')";
                     ...
                     {           
                        while($row=mysqli_fetch_array($r))
                        {
                            ...
                            <tr>
                               ...
                               <td>$row[ddate]</td>
                               <td>$row[noc]</td>  
                               <td><form action='inbound.php' method='post'>
        ...
        if(isset($_POST["submit"])){
            ...
            if($check['total']>0){
                ...
                if ($check_pending['total_pending'] == 0) {
                    ...
                    $query="update ship_employee set task='done',currentdate='$currentdate' where sname='$sname' and ship_no='$ship_no' and e_id='$id'";
                    mysqli_query($con,$query);
                    $sql="update ship set stat='pending' where name='$sname' and stat='managing'";
                    mysqli_query($con,$sql);
                    header("Location:prerecord.php");
                }
            }
