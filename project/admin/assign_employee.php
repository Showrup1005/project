...
<div class="container mt-5">
                ...
                <div class="mb-3">  
                    ...
                    <select name="sname" id="ships" class="form-select">
                        <?php
                            ...
                            $sql = "select name from ship where stat='Checked'";
    ...
    if(isset($_POST['submit'])) {
        if(isset($_POST['sname'])) {
            ...
            $r = mysqli_query($con, $sql);
            if(mysqli_num_rows($r) > 0) {
                echo "<form action='assign_employee.php' method='post'>";
                echo "<div class='table-responsive'>";
                echo "<table class='table table-striped'>";
                echo "<thead><tr><th>Employee Id</th><th>Employee Name</th></tr></thead><tbody>";
                while($row = mysqli_fetch_array($r)) {
                    $e_name = $row['e_name'];
                    $e_id = $row['e_id'];
                    echo "<tr><td><input type='checkbox' value='$e_id' name='e_id[]'>$e_id</td><td>$e_name</td></tr>";
                } 
                echo "</tbody></table>";
                echo "<input type='hidden' name='sname' value='$name'>";
                echo "<div class='text-center'><input name='insert' type='submit' value='Assign' class='btn btn-primary mt-2'></div>";
                echo "</form></div>";  
            } else {
    ...
    if(isset($_POST['insert'])) {
        if(isset($_POST['sname'])) {
            $name=$_POST['sname'];
            $sql="update ship set stat='managing' where name='$name' and stat='Checked'";
            mysqli_query($con,$sql);
            $sql1="select ship_no from ship where name='$name' and stat='managing'";
            ...
            if(mysqli_num_rows($r) > 0) {              
                ...
                foreach($e_id as $employee) {
                    $e_id=$employee;       
                    $sql2="insert INTO ship_employee (e_id,sname,ship_no,task,currentdate) values ('$e_id', '$name', $ship_no, '$task' ,'$currentdate')";
